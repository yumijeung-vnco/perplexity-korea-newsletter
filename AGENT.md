...
```yaml
window_days: 10   # 이 뉴스레터는 kind: role이어도 최근 10일 이내 소식만 다룬다 (오래된 기사 제외)
owner: Perplexity 코리아 브리핑
cadence: 매일 아침 08:00 (Asia/Seoul)
interests:                      # 관심사 = 탭. Perplexity 관련 소식을 4개 주제로 정리하는 PR 목적 뉴스레터.
  # 모든 탭이 kind: role — 속보(직전 24h) 큐레이션이 아니라 대표적인 실제 소식을 큐레이션한다.
  # 공통 제외 원칙: 저작권·소송 등 갈등·분쟁 소재는 어떤 탭에서도 다루지 않는다.
  - id: announcements
    label: Perplexity 공식 발표
    icon: 📣
    tint: blue
    kind: role
    primary_source: https://www.perplexity.ai/hub/blog/category/news   # 최우선 신뢰 소스
    focus: Perplexity 공식 발표(제품·기능·정책 업데이트). perplexity.ai/hub/blog/category/news를 최우선으로 확인하고, 필요시 이를 다룬 매체 보도로 보완한다.
  - id: funding
    label: 투자·기업가치
    icon: 💰
    tint: green
    kind: role
    focus: Perplexity의 투자 유치, 기업가치 평가 등 자금·밸류에이션 관련 소식.
  - id: partnership
    label: 파트너십·제휴
    icon: 🤝
    tint: purple
    kind: role
    focus: Perplexity의 기업 제휴·파트너십 소식. 특히 한국 기업(통신사·제조사 등)과의 협력을 우선한다. 저작권·소송 등 갈등 소재는 다루지 않는다.
  - id: medialens
    label: Media Lens
    icon: 🔎
    tint: orange
    kind: role
    focus: Perplexity의 최근 발표 하나를 골라, 각국(한국 포함) 매체가 이를 어떻게 다르게 보도했는지 비교한다. highlight는 발표 자체, articles는 서로 다른 국가·매체의 보도로 구성한다.
persona_context: |             # '나를 위한 한 줄'을 쓸 때 참고할 독자 맥락
  기자·번역가 출신, AI PR 및 Perplexity 근무 경험이 있으며, 한국 기자들에게
  Perplexity 관련 소식을 정리해 제공하는 PR 브랜드를 만들고 있음.
...
**저작권·소송 등 갈등 소재 제외:** 이 뉴스레터는 PR 목적이므로, 어느 탭에서도 저작권 소송·법적 분쟁 등 갈등성 소재는 다루지 않는다.

**이 뉴스레터의 예외 규칙**: `kind: role` 탭이라도, 이 프로젝트는 시의성이 중요하므로
발행일 기준 최근 10일 이내에 나온 소식만 다룬다. 10일이 넘은 기사는 아무리 좋은
소재라도 제외한다. Media Lens 탭도 비교 대상 발표 자체가 10일 이내여야 한다.
